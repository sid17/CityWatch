<?php
	if (!empty($_POST))
	{
		...
		$height=$obj->{'height'};
		$sql = "select * from reviewdata where zone=".$zone."  order by id desc limit 40";
		$output = "";
		$net = "";
...
$output.="<a href='#zone_info' style='text-decoration:none;font-size:180%;' onclick='update_zone(this)' data-status='".$info['status']."' data-time='".$info['time']."'        data-long='".$info['lang']."' data-lat='".$info['lat']."' id='".$info['id']."' >";
           $output.="<div style='height:100%;width:30%;background-color:white;float:left'><img "; 					   		$output.="src='".$info['image']."' style='width:100%;height:100%'/></div>";
                    		$output.="<div style='height:100%;width:70%;float:right;word-wrap:"; 
